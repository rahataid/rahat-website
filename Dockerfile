...
FROM node:18.20.8-alpine3.18 AS deps
WORKDIR /opt/app
COPY package.json yarn.lock ./
RUN yarn install 

FROM node:18.20.8-alpine3.18 AS builder
WORKDIR /opt/app
COPY . .
COPY --from=deps /opt/app/node_modules ./node_modules
RUN yarn build

FROM node:18.20.8-alpine3.18 AS proddeps
WORKDIR /opt/app
COPY package.json yarn.lock ./
...
FROM node:18.20.8-alpine3.18 AS runner
USER node
WORKDIR /usr/src/app
